# uninstall.ps1 — remove the toast plugin from the DSH user profile.
param(
  [string]$ProfileRoot = ''
)
...
# 2. drop every patch operation block that inserts id toast
if (Test-Path $patchFile) {
  ...
  foreach ($op in $ops) {
    ...
    if ($body -match [regex]::Escape('- id: toast')) { continue }
