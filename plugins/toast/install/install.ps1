# install.ps1 - install the toast plugin into a DSH profile the supported way.
#
...
if ($patchText -notmatch [regex]::Escape('- id: toast')) { throw "cordis.patch.yml does not insert the id 'toast': $patchPath" }
...
# ---------- 2. load check ----------
# The host half is imported by the profile at boot, so a syntax or top-level throw
# takes `dsh web` down with it. Probe it the way the profile does: a script inside
# the profile directory resolves the package by NAME, through its exports map, with
# the profile's own node_modules on the search path. Importing the file path
# directly would prove neither - and would fail outright for a plugin that depends
# on a package the profile provides (@deepseek-ai/*, zod).
#
# An import that merely succeeds also proves little: a module exporting nothing
# imports cleanly and only fails at mount ("plugin.apply is not a function"), so
# assert the plugin contract as well.
$probe = Join-Path $profileDir "_dsh-loadcheck-toast.mjs"
$probeJs = @'
import * as m from '@dsh-user/toast'
...
# seed. Parsing the installed .js with `node --check` would honour package.json's
# "type": "module" and therefore ACCEPT an `import`, so parse a .cjs copy instead.
$clientCjs = Join-Path ([System.IO.Path]::GetTempPath()) "dsh-toast-client-$PID.cjs"
