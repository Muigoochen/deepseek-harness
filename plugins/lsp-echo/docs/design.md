...
- **检查没跑完 ≠ 检查通过**:引擎失败/超时时,改动文件**放回待检查集合**(下一步重试),并注入一句
  「这一轮没跑完,你改的文件没有被验证」+ 原因(同一原因 3 分钟冷却);否则一轮超时会被记成
  "0 错误"。构建型引擎还额外声明 `budgetMs`/`noWait`(见 §9),让 pre-step 通道不被冷启动重编卡住,
  而超时的构建**继续在后台跑**:杀掉 SCons 会扔掉它已完成的工作并把 `.sconsign.dblite` 留在半写状态,
  下一次检查要重编更多(实测 21MB 签名库被打断后,下一次重编 1119 个 godot-cpp 对象);
