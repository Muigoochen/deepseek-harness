...
- **检查没跑完 ≠ 检查通过**:引擎失败/超时时,改动文件**放回待检查集合**(下一步重试,**同一内容最多
  ...
  用于把 pre-step 的自动检查从"一次构建"换成"一次语法检查"。而超时的构建**继续在后台跑**:杀掉 SCons 会扔掉它
...
- **C++ 检查的时间与范围**:`syntax` 阶段用项目自己的编译器查改动文件(实测 1.14 s、不取构建锁、不写
  构建目录),pre-step 的自动检查将带上 `stage: syntax`,于是"改一行 C++ 付一次构建"不再发生。
  `build` 阶段仍是权威结论:模型显式要求检查时编 debug(110s 预算),全量重扫才 debug+release(190s);
