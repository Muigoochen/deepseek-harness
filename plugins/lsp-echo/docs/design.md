...
- **合成键**(`<link>` 这类没有扩展名的条目):引擎在 `engine.json` 里用 `syntheticKeys` 声明归属,
  ...
  记成 `synthetic_keys`(后写的引擎不声明时保留上一份),于是不持有引擎表的读者(GUI 行标签、
  配置变更时的清剪)也能按声明而不是按"名字里有没有点"来判断。没有这条,链接错误会在下一次 `.gd`
...
- **同一构建目录的并发**:宿主在 clientd 超时后会退回一次性 `check`,第二个 DSH 实例也可能在查同一个
  ...
  (不写进项目树),拿不到就等、等不过自己的预算就如实报"另一个检查正在构建";陈旧锁按 pid 存活与
  15 分钟窗口接管。宿主侧同一件事:clientd **答出**的失败是检查结论,不再退回一次性 `check` 重跑
  (否则会在刚被杀掉的构建后面再起一个)。
