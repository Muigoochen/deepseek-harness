...
- **逐 step 变更侦测(Mode B)**:无后台轮询;每个 `pre-step` 对该项目全树 mtime diff,
  只把变更文件推给引擎 → 注入(有错才注入,同文本 3s 节流)。**新建文件同样是"这一步的工作"**:
  mtime diff 只能看见已存在文件的改动,新建文件此前只进 `created`(给引擎做重扫的信号)、从不进
  待检查集合,于是"新建一个 .gd/.cpp 写错"会被静默漏掉 —— 现在 `created` 里属于某个已绑定引擎的
  文件与该引擎的 dirty 文件一起送检;
- **检查没跑完 ≠ 检查通过**:引擎失败/超时时,改动文件**放回待检查集合**(下一步重试,**同一内容最多
  重试 2 次**,按文件内容计数、成功即复位),并注入一句「这一轮没跑完,你改的文件没有被验证」+ 原因
  (同一原因 3 分钟冷却);否则一轮超时会被记成"0 错误"。构建型引擎还额外声明 `budgetMs`/`noWait`
  (见 §9),让 pre-step 通道不被冷启动重编卡住;C++ 引擎另有 `syntax` 阶段(项目自己的编译器、秒级),
  pre-step 发的是 `auto`(`limits.stage = eng.preStepStage`,取值由 `engine.json` 声明):跑得动就是一次
  语法检查,本机驱动不了时才退回构建。而超时的构建**继续在后台跑**:杀掉 SCons 会扔掉它
  已完成的工作并把 `.sconsign.dblite` 留在半写状态,下一次检查要重编更多(实测 21MB 签名库被打断后,
  下一次重编 1119 个 godot-cpp 对象)。这条"后台跑完"是**常驻 `clientd` 通道**的保证(它还活着,
  继续读构建输出);一次性 `check` 交出孤儿锁记录后自己退出,它的构建在父进程退出后若还要写输出
  可能因管道关闭而中断 —— 所以插件优先走 clientd,一次性路径只是退路;
...
引擎不必有常驻进程:`cpp-gdextension` 用项目自己的工具链当引擎,分两级 —— `syntax`(项目自己的
编译器 + 项目自己的 flags,`g++ -fsyntax-only` / `cl /Zs`,不构建、不取锁、约 1 秒)与 `build`
(项目真编一次,唯一能看见链接错误、构建脚本与依赖库问题的阶段)。`stage` 随请求传(默认 `build`,
与历史行为一致;`auto` = 优先 syntax、跑不了就退回 build),`host/stop` 是预检与空操作,`clientd`
把两种请求分别串行化(`lib/manager.js` 的 clientd 契约不变,payload 形状与 godot/typescript 桥一致)。
为什么语法阶段不用 clangd:见 `checkers/cpp-gdextension/README.md` —— 同一个编译器才不会和构建互相
矛盾,而 clangd 要 CDB + `--query-driver` + PATH 三件事同时成立,实测配错一次报出 24 条假错误。
...
- **C++ 语法阶段的前提与诚实边界**:编译器按 `CXX` → `MINGW_BIN` → **项目构建脚本里写死的路径**
  (实测本项目 `build.ps1` 里的 MinGW 绝对路径就是这样找到的) → PATH 找;`CXX`/`MINGW_BIN` 只在
  家族与项目一致时才被采纳(否则 flags 是 MSVC 的、编译器是 MinGW 的,两边都不可信);MSVC 项目找不到
  `cl` 时**退回**机器上有的 g++,并把 flags 一起切到 MinGW(工具链与 flags 永远同源)。学到的 flags 按
  **编译器家族**记账(`cl` = MSVC,其余 = MinGW,取自那行命令本身,而不是产物推断出的标签),家族不符的
  `compile_commands.json` 会被忽略并记下原因。一次**没有实际编译任何文件**的构建不教任何东西(记录按下一次
  真实编译的命令行;实测本项目当时已是最新,所以仍用布局推断),记录按签名存活,因此只要学到过一次就一直有效。
  编译器缺失、spawn 失败、
  **退出码非 0 却没有一条能归到源文件的诊断**(不认的 flag、坏掉的工具链)→ 一律 exit 2 且**不写
  payload**;绝不能因为"没解析出错误"就报 0 错误。语法阶段只看语法与语义,链接/构建脚本/依赖库不在
  它范围内(`engine_note` 每次写明),它也不产生 `<link>` 记录。`budgetMs`/`--build-timeout-ms` 是整个
  阶段的截止时间(不是每个文件一份),超了以 exit 2 报跑到哪里;只认落在该构建目录、且解析回同一构建
  的文件,另一个 GDExtension 的文件计进"本次没有检查"。
  MSVC 分支另有两个实测坑:必须用 `windowsVerbatimArguments` 把命令行原样交给 `cmd.exe`(Node 默认的
  反斜杠转义会让 `call vcvars && cl /Zs` 静默失败、退出码非 0 却没有任何输出),而 `cl` 出错时的退出码是
  **2** 而不是 gcc 的 1 —— 阶段因此把"非 0 退出码、却解析不出一条能归到源文件的诊断"统一当作跑不了,
  而不是把某一家编译器的退出码当作约定。
- **C++ 检查的时间与范围**:`syntax` 阶段用项目自己的编译器查改动文件(实测 1.14 s、不取构建锁、不写
  构建目录),pre-step 的自动检查带 `stage: auto`(插件把它放进 `limits`,值来自 `engine.json` 的
  `preStepStage`):跑得动就"改一行 C++ 付一次构建"不再发生,驱动不了时退回构建。`build` 阶段仍是权威结论:模型显式要求检查时编 debug(110s 预算),全量重扫才 debug+release(190s);
  **自动(pre-step)通道另有 40 秒预算**(引擎自述 `budgetMs`)且不排队等别的构建 —— 一轮对话不会被冷启动
  重编卡住(`noWait`:桥若正忙就立刻作答,clientd 也不把这种请求排进队列,否则宿主的短超时会先到、
  把整个 clientd 连同在跑的那个请求一起退休)。超预算的检查把改动文件交还待检查集合,但**同一文件内容
  最多重试 2 次**(超过就不再自动重试,注入的提示也如实改口),免得一个比预算慢的构建每一步都付一次预算。
...
- **快照合并的粒度是一条记录,不是一个扩展名**(2026-09-25 改):一次写入**只替换它自己带的键**,
  没带的键原样保留 —— 另一个引擎的键空间、以及本引擎这轮没看的文件的旧记录都不动;只有配置里不再
  绑定的扩展名、以及文件已经不存在的旧记录才会被清掉。理由:C++ 的语法阶段只查改动过的文件,按
  「整个扩展名替换」会把同项目其它 `.cpp` 的旧记录、连同 `<link>` 一起抹掉,而"没有记录"在下游
  (GUI 行、汇总计数、模型看到的诊断)读起来就是"没有错误"。
...
- **同一构建目录的并发**:宿主在 clientd 超时后会退回一次性 `check`,第二个 DSH 实例也可能在查同一个
  ...
  失败)。**预算到点默认不杀构建**:杀掉 SCons 会扔掉它已做完的工作、把 `.sconsign.dblite` 留在半写状态,
  下一次检查要重编更多(实测被打断后下一次重编 1119 个 godot-cpp 对象、约 112 秒,而正常增量 4.4 秒),
  所以桥把锁交给那个还在跑的构建(记成"孤儿"记录:只记 `childPid`、不带取锁 pid)并立刻作答,下一次检查
  等它、或如实报它,而不是在旁边再开一个;要真杀得显式 `--kill-on-timeout`,那条路才走
  `taskkill /T /F`(POSIX 进程组)+ 5 秒宽限,宽限到点也照样作答,并把锁留成同样的孤儿记录。
  一次性 `check` 在交出孤儿记录后要能退出:构建子进程还握着管道,所以桥在这条路上摘掉 stdio 监听并
  `unref` 它们(否则进程要等构建结束,宿主就得为一份已经打印出来的结论付满超时)。
  宿主侧同一件事:clientd **答出**的失败是检查结论,不再退回一次性 `check` 重跑(否则会在刚被杀掉的构建
  后面再起一个);一次性路径把文件放在 `check` 之后、所有开关之前(无值开关后面跟路径会把那个文件吃掉)。
  快照里的引擎自述只有一个槽位:后写的引擎覆盖,它自己 payload 里没有就清空 —— 所以它只会消失,不会伪装
  成别的引擎的话。
  已知边界:锁只在共享一个 `DSH_HOME` 的进程之间有效,且按构建目录分锁,两个项目共用一份 `godot-cpp`
  检出时仍可能各编各的(刻意不做全局串行);`--kill-on-timeout` 若杀掉了具名子进程而孙进程仍握着管道,
  孤儿记录里的 `childPid` 已死、下一次检查会接管并紧挨着那个存活进程再编一次 —— 插件自己从不发这个开关
  (只有探针和手动 CLI 用),所以是一条 opt-in 的已知边界,不是当前路径上的行为。
