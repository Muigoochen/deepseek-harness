...
# profile and reconciles its bundle list. Do NOT delete the profile's node_modules
# entry by hand: while the plugin is installed that path is a junction back to the
# checkout, and a recursive delete through a junction can take the checkout's files
# with it.
#
...
if (Test-Path $patchFile) {
  ...
  $idNeedle = "(?m)^\s*- id: lsp-echo\s*$"
  $nameNeedle = "name: '@dsh-user/lsp-echo'"
  ...
  foreach ($op in $ops) {
    ...
    if ($body -cmatch $idNeedle -or $body -cmatch ([regex]::Escape($nameNeedle))) { $removed = $true; continue }
    for ($j = $op.Start; $j -lt $op.End; $j++) { $kept.Add($lines[$j]) }
  }
  for ($j = $cursor; $j -lt $lines.Count; $j++) { $kept.Add($lines[$j]) }
  # Only write when a row was actually found. Rewriting unconditionally would strip the
  # file's header comments, and would empty a patch that has no `- insert:` operation at all.
  if ($removed) {
    ...
    Write-Host "no lsp-echo row found in: $patchFile"
  }
} else {
