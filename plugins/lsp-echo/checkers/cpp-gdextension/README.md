...
时间从里面扣;如果等到剩余不足 10 秒,就提前拒绝(而不是起一个注定要被杀掉的构建)。所以
桥一定在自己的预算内作答,不会拖到宿主那边的 clientd 计时器之后 —— 那种"宿主杀掉 clientd、
构建变成孤儿、重试又被锁挡住"的连锁就是这条规则要避免的。
...
```powershell
# 诊断解析 / 协议 / 诚实性路径 / 每文件构建目录 / 并发构建锁 + 真实 MSVC 端到端(63 项)
