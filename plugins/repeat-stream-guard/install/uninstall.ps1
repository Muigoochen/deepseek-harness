# uninstall.ps1 - remove repeat-stream-guard from the DSH user profile.
...
# 2. drop every patch operation block that inserts id repeat-stream-guard
if (Test-Path $patchFile) {
  ...
  foreach ($op in $ops) {
    ...
    if ($body -match [regex]::Escape('- id: repeat-stream-guard')) { continue }
