# uninstall.ps1 - remove repeat-stream-guard from a DSH profile.
#
...
$removeOut = (& $pnpmExe dsh plugin --profile $ProfileName remove '@dsh-user/repeat-stream-guard' 2>&1 | Out-String)
...
if (Test-Path $patchFile) {
  ...
  $idNeedle = "(?m)^\s*- id: repeat-stream-guard\s*$"
  $nameNeedle = "name: '@dsh-user/repeat-stream-guard'"
  ...
  if ($removed) {
    ...
    Write-Host "removed the repeat-stream-guard row from: $patchFile"
  } else {
    Write-Host "no repeat-stream-guard row found in: $patchFile"
  }
} else {
