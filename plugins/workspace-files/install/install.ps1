# install.ps1 - install workspace-files into a DSH profile the supported way.
#
...
param(
  [string]$PluginSource = (Join-Path $PSScriptRoot '..'),   # plugins/workspace-files
...
$probe = Join-Path $profileDir "_dsh-loadcheck-workspace-files.mjs"
$probeJs = @'
import * as m from '@dsh-user/workspace-files'
...
$clientCjs = Join-Path ([System.IO.Path]::GetTempPath()) "dsh-workspace-files-client-$PID.cjs"
...
Write-Host 'verify: the sidebar shows the workspace file tree and dropping a file inserts an @relative reference into the composer.'
