# install.ps1 — install the workspace-files plugin into the DSH user profile.
...
$needle = 'name: ''@dsh-user/workspace-files'''
...
"@
  ...
  Write-Host "appended workspace-files row to: $patchFile"
}

Write-Host ''
Write-Host 'installed. Restart `dsh web` to activate (a running instance keeps its host rows).'
Write-Host 'verify: open a workspace page (no startup errors), then the sidebar-foot button opens the file-tree drawer.'
