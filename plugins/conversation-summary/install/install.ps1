# install.ps1 - install conversation-summary into a DSH profile the supported way.
#
...
#
# This plugin's budget, trigger and mode choices are user settings, so they belong
# in $DSH_HOME/profiles/<name>/cordis.patch.yml, where a row with the same id is
# applied after this bundle's layer and wins. See install/patch.example.yml.
#
...
if ($patchText -notmatch [regex]::Escape('- id: conversation-summary')) { throw "cordis.patch.yml does not insert the id 'conversation-summary': $patchPath" }
...
# on the search path - which is where this plugin's dependencies (zod,
# @deepseek-ai/dsh-llm, @deepseek-ai/dsh-compaction) actually live.
$probe = Join-Path $profileDir "_dsh-loadcheck-conversation-summary.mjs"
$probeJs = @'
import * as m from '@dsh-user/conversation-summary'
...
$clientCjs = Join-Path ([System.IO.Path]::GetTempPath()) "dsh-conversation-summary-client-$PID.cjs"
...
Write-Host 'verify: the settings page shows the conversation-compaction section, and compact_conversation is available to the agent.'
