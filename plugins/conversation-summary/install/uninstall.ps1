# uninstall.ps1 - remove conversation-summary from a DSH profile.
#
...
# junction back to the checkout, and a recursive delete through a junction takes
# the checkout's files with it.
#
...
$removeOut = (& $pnpmExe dsh plugin --profile $ProfileName remove '@dsh-user/conversation-summary' 2>&1 | Out-String)
...
# The row you tuned (budget, triggers, mode) lives here, so removal takes it with it.
if (Test-Path $patchFile) {
  $lines = [System.IO.File]::ReadAllLines($patchFile)
  $idNeedle = "(?m)^\s*- id: conversation-summary\s*$"
  $nameNeedle = "name: '@dsh-user/conversation-summary'"
  ...
  if ($removed) {
    ...
    Write-Host "removed the conversation-summary row from: $patchFile"
  } else {
    Write-Host "no conversation-summary row found in: $patchFile (the bundle layer supplies it)"
  }
} else {
